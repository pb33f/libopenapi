...
| `type: object` with no properties | `Record<string, unknown>`, or `Record<string, never>` with `additionalProperties: false` |
| no type | `unknown` |
| `$ref` to a component | the component's type name |
| `$ref` into a component (`…/properties/p`, `/items`, `/additionalProperties`) | indexed access through non-nullable containers, `Exclude<NonNullable<X>["p"], undefined>` |
...
Reserved words, invalid identifiers and the globals the output uses (`Array`,
`Exclude`, `NonNullable`, `Record`) are renamed and reported with

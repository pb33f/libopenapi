...
func TestSchema_buildAllOfFromTransformedNode_InvalidStructure(t *testing.T) {
	schema := &Schema{}

	// test with node that doesn't have allOf as first element
	var node yaml.Node
	_ = yaml.Unmarshal([]byte(`title: "Test"`), &node)

	err := schema.buildAllOfFromTransformedNode(node.Content[0])
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "invalid allOf structure")
}

func TestSchema_buildAllOfFromTransformedNode_NotEnoughContent(t *testing.T) {
	schema := &Schema{}

	// test with node that has allOf but no value
	node := &yaml.Node{
	...
	err := schema.buildAllOfFromTransformedNode(node)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "invalid allOf structure")
}

func TestSchema_buildAllOfFromTransformedNode_NotArray(t *testing.T) {
	schema := &Schema{}

	// test with allOf value that's not an array
	node := &yaml.Node{
		Kind: yaml.MappingNode,
		Content: []*yaml.Node{
			{Kind: yaml.ScalarNode, Value: "allOf"},
			{Kind: yaml.ScalarNode, Value: "not an array"},
		},
	}

	err := schema.buildAllOfFromTransformedNode(node)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "allOf value is not an array")
}

func TestSchema_buildAllOfFromTransformedNode_ValidAllOf(t *testing.T) {
	schema := &Schema{}
	schema.context = context.Background()

	// create index for the schema proxy building
	config := index.CreateOpenAPIIndexConfig()
	idx := index.NewSpecIndexWithConfig(&yaml.Node{}, config)
	schema.index = idx

	// test with valid allOf structure
	node := &yaml.Node{
		Kind: yaml.MappingNode,
		Content: []*yaml.Node{
			{Kind: yaml.ScalarNode, Value: "allOf"},
				...
				Content: []*yaml.Node{
					{
						...
						Content: []*yaml.Node{
							...
							{Kind: yaml.ScalarNode, Value: "Test Schema"},
						},
					},
					{
						Kind: yaml.MappingNode,
	...
	// verify AllOf field was populated
	assert.False(t, schema.AllOf.IsEmpty())
	assert.Len(t, schema.AllOf.Value, 2)

	// verify key and value nodes are set correctly
	assert.Equal(t, "allOf", schema.AllOf.KeyNode.Value)
	assert.Equal(t, yaml.SequenceNode, schema.AllOf.ValueNode.Kind)
}

func TestSchema_buildAllOfFromTransformedNode_EmptyArray(t *testing.T) {
	schema := &Schema{}
	schema.context = context.Background()
	...
	// verify AllOf field was populated but empty
	assert.False(t, schema.AllOf.IsEmpty())
	assert.Len(t, schema.AllOf.Value, 0)
}
